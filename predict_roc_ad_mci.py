...
train_data = scipy.io.loadmat(cur_filepath)  # 读入存放数据的字典
x_test = np.array(train_data['x_train'])#x_test = np.array(train_data['x_test'])
y_test = np.array(train_data['y_train'])#y_test = np.array(train_data['y_test'])
img_rows, img_cols, img_depth = 61, 73, 61
if K.image_data_format() == 'channels_first':
    print("i am first")
...
nc_path = os.path.join(VALID_DIR,'AD')
...
y_score = prediction[:,0]   # NC的标签为1，以NC为正例，这个弄清楚非常重要
